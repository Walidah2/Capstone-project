...
def load_bundle():
    ...
    threshold_default: float = float(bundle.get("threshold_default", 0.40))  # fixed in backend
...
with st.container():
    ...
    with st.form("single_row"):
        ...
        # Visible categoricals (Title/Deadline hidden)
        c1, c2 = st.columns(2)
        with c1:
        ...
        # Auto-fill note (after time input)
        st.markdown(
        ...
        ui["Salary"] = salary_default  # pass to the model
    ...
    if submitted:
        ...
        thr_fixed = float(threshold_default)  # fixed backend threshold (e.g., 0.40)

        if hasattr(pipe, "predict_proba"):
            p = float(pipe.predict_proba(X)[:, 1][0])
            pred = int(p >= thr_fixed)
            st.metric("Probability (class = 1)", f"{p:.4f}")
            st.success(f"**Prediction:** {pred}  —  threshold fixed at {thr_fixed:.2f}")
        else:
    ...
    if uploaded:
        ...
        with st.spinner("Scoring..."):
            if hasattr(pipe, "predict_proba"):
                ...
                thr_fixed = float(threshold_default)  # same fixed threshold
                df_scored["prediction"] = (df_scored["proba"] >= thr_fixed).astype(int)
            else:
